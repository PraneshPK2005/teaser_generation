...
def get_teaser_timestamps(transcription_data: Union[List[str], str], expected_time: int, max_sec: int) -> List[List[float]]:
    """
    Extracts the most important timestamps from a transcript for a teaser with specified duration.
    
    Args:
        transcription_data: Either a list of timestamped transcript lines or a single string
        expected_time: Minimum duration of the teaser in seconds
        max_sec: Maximum duration of the teaser in seconds
    
    Returns:
        List of important segments as [start, end] pairs
    """
    ...
    # More specific prompt with duration constraints
    prompt = f"""
ANALYSIS TASK:
You are analyzing a video transcript to create a {expected_time}-{max_sec} second teaser. 
Identify the most important/crucial moments that would make an engaging teaser.

TRANSCRIPT:
{transcription_text}

INSTRUCTIONS:
1. Select only the most impactful moments that tell the core story
2. Focus on moments with high emotion, key plot points, or dramatic reveals
3. Select segments whose combined duration is between {expected_time} and {max_sec} seconds
4. Return ONLY the timestamps in the exact format: [START_END] (one per line)
5. All timestamps must be in seconds
6. The END time must be greater than the START time
7. Do not include any explanations or additional text
8. Use the exact timestamps from the transcript, don't create new ones
9. Do not modify the original timestamps in any way
...
"""
    ...
    try:
        ...
        # Convert to the required format: [[start, end], [start, end], ...]
        formatted_segments = [[segment["start"], segment["end"]] for segment in segments]
...
def parse_timestamps_from_response(response_text: str) -> List[Dict]:
    """
    Parses timestamps from the model response text with better error handling.
    
    Args:
        response_text (str): The response text from the model
        
    Returns:
        List of timestamp segments with 'start' and 'end' values
    """
        ...
        for match in re.finditer(pattern, response_text):
            try:
                if len(match.groups()) >= 2:
                    ...
                    if ':' in match.group(0):  # Colon format (minutes:seconds)
                        minutes = float(start_time_str)
                        seconds = float(end_time_str)
                        start = minutes * 60 + seconds
                        end = start + 10  # Assume 10-second segments for single timestamps
                    else:  # Second-based format
                ...
                print(f"Error parsing timestamp {match.group(0)}: {e}")
                continue
